<template>
              ...
              <a-form ref="completeRef" :colon="false" :model="completeForm" layout="vertical" style="padding: 20px">
                ...
                <a-row :gutter="[24, 0]">
                  <a-col v-for="n in 16" :key="n" :span="12">
                    <a-form-item>
                      <template #label>
                        <span style="cursor:pointer" @click="openPhotoRule(n)">配置照片{{ n }}</span>
                        <span style="margin-left:6px;color:#8c8c8c;font-size:12px;cursor:pointer;white-space:nowrap" @click="openPhotoRule(n)">要求<RightOutlined style="font-size:10px;margin-left:2px;vertical-align:middle" /></span>
                      </template>
                      <a-upload
  ...
  <!-- 工程照片审核要求 Drawer（全部16项） -->
  <a-drawer
    v-model:open="photoRuleOpen"
    title="工程照片审核要求"
    :width="480"
    placement="right"
    @after-open-change="onPhotoRuleDrawerOpen"
  >
    <div v-for="n in 16" :key="n" :id="`photo-rule-${n}`"
         :style="{ marginBottom: '32px', scrollMarginTop: '8px', borderTop: n > 1 ? '1px solid #f0f0f0' : 'none', paddingTop: n > 1 ? '28px' : '0' }">
      <div style="font-size:14px;font-weight:600;color:rgba(0,0,0,0.88);margin-bottom:12px">
        {{ PHOTO_RULES[n].title }}
      </div>
      <!-- 上传要求 -->
      <div style="font-size:12px;font-weight:600;color:#8c8c8c;letter-spacing:.5px;margin-bottom:6px;text-transform:uppercase">上传要求</div>
      <p style="color:rgba(0,0,0,0.65);font-size:13px;line-height:1.7;margin:0 0 8px">{{ PHOTO_RULES[n].upload.desc }}</p>
      <a-image-preview-group>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
          <a-image v-for="(img, i) in PHOTO_RULES[n].upload.images" :key="i"
            :src="img" :width="120" :height="90" style="object-fit:cover;border-radius:4px;border:1px solid #f0f0f0;display:block" />
        </div>
      </a-image-preview-group>
      <!-- 审核要求 -->
      <div style="font-size:12px;font-weight:600;color:#8c8c8c;letter-spacing:.5px;margin-bottom:6px;text-transform:uppercase">审核要求</div>
      <p style="color:rgba(0,0,0,0.65);font-size:13px;line-height:1.7;margin:0 0 8px">{{ PHOTO_RULES[n].review.desc }}</p>
      <a-image-preview-group>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <a-image v-for="(img, i) in PHOTO_RULES[n].review.images" :key="i"
            :src="img" :width="120" :height="90" style="object-fit:cover;border-radius:4px;border:1px solid #f0f0f0;display:block" />
        </div>
      </a-image-preview-group>
    </div>
...
import {
  ...
  DownOutlined, UploadOutlined, PlusOutlined, QuestionCircleOutlined, CheckCircleFilled, RightOutlined,
...
const photoRuleOpen   = ref(false)
const photoRuleTarget = ref(1)
const PHOTO_RULES: Record<number, { title: string; upload: { desc: string; images: string[] }; review: { desc: string; images: string[] } }> = {
  1: { title: '配置照片1 — 屋面全景', upload: { desc: '拍摄整体屋面，须包含所有组件阵列，无遮挡，光线均匀。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '画面清晰、无过曝，组件与支架可辨，拍摄角度能反映实际布局。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  2: { title: '配置照片2 — 组件安装', upload: { desc: '近景拍摄已安装组件的支架固定点，展示螺栓紧固情况。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '螺栓规格符合图纸要求，压块位置正确，防腐处理到位，图像清晰。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  3: { title: '配置照片3 — 逆变器铭牌', upload: { desc: '拍摄逆变器铭牌，确保序列号、型号清晰可读。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '铭牌无遮挡，序列号与系统录入完全一致，文字不模糊。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  4: { title: '配置照片4 — 并网柜正面', upload: { desc: '拍摄并网柜正面，柜门打开展示内部接线。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '断路器型号可见，线缆标签齐全，接地线连接规范。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  5: { title: '配置照片5 — 计量表/电表', upload: { desc: '拍摄计量表正面，表号与读数清晰可见。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '表号与合同一致，读数清晰，封签完好，无反光遮挡。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  6: { title: '配置照片6 — 汇流箱', upload: { desc: '拍摄汇流箱内部，展示各路输入保险及线缆连接。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '各路保险编号标注清晰，线缆无裸露，防水接头紧固。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  7: { title: '配置照片7 — 直流线缆敷设', upload: { desc: '拍摄直流线缆在屋面的整体走向，穿管防护情况。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '穿管无破损，卡扣固定间距 ≤ 1m，标签标识清晰。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  8: { title: '配置照片8 — 交流线缆敷设', upload: { desc: '拍摄从逆变器至并网柜的交流线缆路径。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '线槽封盖完整，弯曲半径符合要求，接线盒防水处理到位。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  9: { title: '配置照片9 — 防雷接地', upload: { desc: '拍摄支架接地及防雷连接点，接地极位置。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '接地导线截面积达标，接地极与支架连接可见，防腐处理到位。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  10: { title: '配置照片10 — 监控设备', upload: { desc: '拍摄数据采集器/监控主机安装位置及信号灯状态。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '指示灯状态正常（绿灯），SIM 卡已安装，天线无遮挡。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  11: { title: '配置照片11 — 施工现场环境', upload: { desc: '拍摄施工区域整体环境，无杂物堆积。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '现场无建筑垃圾，安全围栏到位，警示标志齐全。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  12: { title: '配置照片12 — 屋面防水处理', upload: { desc: '拍摄穿孔及支架底座防水密封情况。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '密封胶饱满无空鼓，防水涂料覆盖完整，无积水痕迹。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  13: { title: '配置照片13 — 设备标识牌', upload: { desc: '拍摄逆变器、并网柜等设备安全标识牌。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '标识牌固定牢固，内容清晰无褪色，位置符合规范。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  14: { title: '配置照片14 — 完工验收现场', upload: { desc: '拍摄业主代表与施工负责人现场验收合影。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '合影须包含项目铭牌，日期可确认，人员着安能安全帽。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  15: { title: '配置照片15 — 并网调试截图', upload: { desc: '拍摄并网成功后监控平台或逆变器屏幕截图，显示发电功率。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '功率数据非零，时间戳可见，显示屏无反光遮挡。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
  16: { title: '配置照片16 — 其他补充', upload: { desc: '如有特殊施工情况，可在此上传补充说明照片，并在备注中注明。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] }, review: { desc: '可上传多张，文件大小 ≤ 10MB/张，格式支持 JPG / PNG。', images: ['data:image/svg+xml;base64,', 'data:image/svg+xml;base64,'] } },
}
function scrollToPhotoRule() {
  nextTick(() => {
    document.getElementById(`photo-rule-${photoRuleTarget.value}`)
      ?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  })
}
function onPhotoRuleDrawerOpen(open: boolean) {
  if (open) scrollToPhotoRule()
}
function openPhotoRule(n: number) {
  photoRuleTarget.value = n
  if (photoRuleOpen.value) {
    scrollToPhotoRule()
  } else {
    photoRuleOpen.value = true
  }
}
